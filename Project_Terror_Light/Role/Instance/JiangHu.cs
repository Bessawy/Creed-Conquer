...
namespace Project_Terror_Light.Role.Instance
{
    public class JiangHu : IEnumerable<JiangHu.Stage>
    {
        ...
        {
            byte first = (byte)Rand.Next((tryingultra > 0) ? 3 : 1, 7);
            if (tryingultra > 0)
                tryingultra -= 1;
            if (first >= 6)
            {
                first = (byte)Rand.Next(1, 7);
                if (first > 6)
                    first = 6;
            }
            return first;
        ...
        internal void CreateRollValue(ServerSockets.Packet stream, Client.GameClient client, byte mStar, byte mStage)
        {
            ...
            Buckupstar.Star.Level = CreateStarLevel(client.Player.JingTryngUltra);

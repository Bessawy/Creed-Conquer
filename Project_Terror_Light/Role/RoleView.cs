...
namespace Project_Terror_Light.Role
{
    public class RoleView
    {
        ...
        public unsafe void CheckScrennColors(ServerSockets.Packet stream)
        {
            ...
            {
                if (Owner.Player.Y < 173)//Project_Terror_Light.Role.Core.GetDistance(Owner.Player.X, Owner.Player.Y, 257, 140) < 18)
                {
                    InColorScrenn = true;
                ...
                using (var rec = new ServerSockets.RecycledPacket())
                {
                    ...
                    try
                    {
                        if (Database.HouseTable.InHouse(Owner.Player.Map))
                        {
                            ...
                            {
                                Project_Terror_Light.Role.Instance.House House;
                                if (Project_Terror_Light.Role.Instance.House.HousePoll.TryGetValue(Owner.Player.DynamicID, out House))
                                {
                                    try
                                    {

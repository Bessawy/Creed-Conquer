...
namespace Project_Terror_Light.Role
{
    public unsafe class Player : IMapObj
    {
#if msgtick
        public int LastClientTick = 0;
        public int LastRcvClientTick = Environment.TickCount;
        public int LastReqClientTick = Environment.TickCount;
#endif
#if MiningSystem
...
550$ 10000 InnerPower*/
        ...
        {
            if (HistoryChampionPoints >= 343700)
            {
                return;
            }
            if (settodayvalue)
   ...
   , 0, 0, Game.MsgServer.MsgUpdate.DataType.AzureShield, true);
        ...
        public unsafe void Dead(Role.Player killer, ushort DeadX, ushort DeadY, uint KillerUID)
        {
            ...
            if (Game.MsgTournaments.MsgSchedules.ArenaCpsBattle.InTournament(Owner) && Game.MsgTournaments.MsgSchedules.ArenaCpsBattle.InTournament(killer.Owner))
            {
                Game.MsgTournaments.MsgSchedules.ArenaCpsBattle.End(killer.UID, UID);
                ...
                if (Owner.FullLoading)
                {
                    ...
                    else if (Database.AtributesStatus.IsNinja(Class))
                    {
                        ...
                        if (_level < 40 && value >= 40)
                        {
                            ...
                            {
                                var stream = rec.GetStream();
                                if (!Owner.MySpells.ClientSpells.ContainsKey((ushort)Role.Flags.SpellID.SuperTwofoldBlade))
                                    Owner.MySpells.Add(stream, (ushort)Role.Flags.SpellID.SuperTwofoldBlade);
        ...
        public long Money
        {
            ...
            set
            {
                ...
                {
                    long dif = value - 50000000000;
                    WHMoney += dif;
                    value -= dif;
                    if (Owner.PokerPlayer != null)
                        Owner.PokerPlayer.CurrentMoney = value;
                    this.Owner.SendSysMesage(dif + " Gold has transfare to your warehouse you can't hold more than 50,000,000,000 into your inventory.", MsgMessage.ChatMode.Monster, MsgMessage.MsgColor.yellow);
                }
                _Money = value;
                using (var rec = new ServerSockets.RecycledPacket())
                {
        ...
        public void AddVipVote(double Minutes)
        {
            ...
            Owner.CreateBoxDialog("Thank you for your support. You`ve received vip6 (1 hour).");
        }

...
using Project_Terror_Light.Game.MsgServer.AttackHandler;

namespace Project_Terror_Light.Client
{
    public class PoolProcesses
    {
                                    ...
                                    using (var rec = new ServerSockets.RecycledPacket())
                                    {
                                        ...
                                        foreach (var spell in spellclient.Spells.GetValues())
                                        {
                                            if (spellclient.CheckInvocke(Now, spell))
                                            {
                                                ...
                                                //if (spellclient.AttackedTwilight == 0)
                                                {
                                                    spellclient.SendView(stream, client);
                                                }
                                                //spellclient.AttackedTwilight += 1;
                                                ActionQuery action = new ActionQuery()
                                                {
                                                    ObjId = spell.FloorPacket.m_UID,
                                                    dwParam_Hi = spell.FloorPacket.m_Y,
                                                    dwParam_Lo = spell.FloorPacket.m_X,
                                                    wParam1 = spellclient.AttackedPOSX,//(ushort)(spell.FloorPacket.m_X - 2),
                                                    wParam2 = spellclient.AttckedPOSY, //(ushort)(spell.FloorPacket.m_Y - 3),
        ...
        public static unsafe void CheckSecounds(Client.GameClient client)
        {
            try
            {
                if (client == null || !client.FullLoading || client.Player == null || client.Player.CompleteLogin == false)
                    return;
#if msgtick
                if (Environment.TickCount - client.Player.LastRcvClientTick > 15000)
                {
                    client.Socket.Disconnect();
                    return;
                }
                if (Environment.TickCount - client.Player.LastReqClientTick >= 10000)
                {
                    using (var rec = new ServerSockets.RecycledPacket())
                    {
                        var stream = rec.GetStream();
                        client.Send(stream.CreateTickData(client.Player));
                    }
                }
#endif
                if (client.Player.Map == 10166 || client.Player.Map == 1005 || client.Player.Map == 1860 || client.Player.Map == 1858 || client.Player.Map == 700 || client.Player.Map == 1004 || Game.MsgTournaments.MsgSchedules.CurrentTournament.InTournament(client))
                {
                ...
                Database.VoteSystem.SendCheckUp(client);
        ...
        public static void StampXPCountCallback(Client.GameClient client)
        {
            try
            {
                if (client == null || !client.FullLoading || client.Player == null || client.Player.OnAutoHunt)
                    return;
        ...
        public static void StaminaCallback(Client.GameClient client)
        {
            try
            {
                ...
                if (!client.Player.Alive)
                    return;
                if (Now >= client.Player.HuntingBounsStamp)
                {
                    if (client.Player.HuntingBounsLayer > 0) { client.Player.HuntingBounsStamp = Now.AddMinutes(5); client.Player.HuntingBounsLayer -= 1; }
                }
                    ...
                    else if (flag.Key == (int)Game.MsgServer.MsgUpdate.Flags.DragonFlow)
                    {
                        if (flag.CheckInvoke(Timer))
                        {
                            ...
                            if (client.Player.Stamina < MaxStamina)
                            {
                                ...
                                {
                                    var stream = rec.GetStream();
                                    client.Player.SendString(stream, MsgStringPacket.StringID.Effect, true, "leedragonblood");
                                    client.Player.SendUpdate(stream, client.Player.Stamina, Game.MsgServer.MsgUpdate.DataType.Stamina);
                                }
                            }
                        }
        ...
        public static unsafe void MiningProcess(Client.GameClient client)
        {
            if (!client.Player.Mining)
                return;
            if (!client.Map.TypeStatus.HasFlag(Project_Terror_Light.Role.MapTypeFlags.MineEnable))
            {
            ...
            using (var recycle = new ServerSockets.RecycledPacket())
            {
                ...
                Project_Terror_Light.Database.MineRule Role = null;
                if (Project_Terror_Light.Database.MiningTable.GetRandomRole(out Role))
                {
                    switch (Role.RuleType)
                    {
                        case Database.MineRuleType.MineOre:
                            {
                                client.Inventory.Add(stream, Role.RuleValue, 1, 0, 0, 0, Project_Terror_Light.Role.Flags.Gem.NoSocket, Project_Terror_Light.Role.Flags.Gem.NoSocket, false, Project_Terror_Light.Role.Flags.ItemEffect.None, true, "!", 0, 0, 0, true);
                                client.Player.MiningAttempts--;
                                break;
                            }
                        case Database.MineRuleType.MineGem:
                            {
                                uint ItemID = Role.RuleValue;
                                if (ItemID % 10 == 1)
                                {
                                    if (Database.MiningTable.PercentSuccess(10.0))
                                        ItemID += 1;
                                    if (ItemID % 10 == 1)
                                    {
                                        if (Database.MiningTable.PercentSuccess(1.0))
                                            ItemID += 2;
                                    }
                                    if (ItemID % 10 == 2)
                                    {
                                ...
                                client.Inventory.Add(stream, ItemID, 1, 0, 0, 0, Project_Terror_Light.Role.Flags.Gem.NoSocket, Project_Terror_Light.Role.Flags.Gem.NoSocket, false, Project_Terror_Light.Role.Flags.ItemEffect.None, true, "~from~mining!", 0, 0, 0, true);
                                client.Player.MiningAttempts--;
                                break;
                            }
                        case Database.MineRuleType.MineMisc:
                            {
                                client.Inventory.Add(stream, Role.RuleValue, 1, 0, 0, 0, Project_Terror_Light.Role.Flags.Gem.NoSocket, Project_Terror_Light.Role.Flags.Gem.NoSocket, false, Project_Terror_Light.Role.Flags.ItemEffect.None, true, "~from~mining!", 0, 0, 0, true);
                                client.Player.MiningAttempts--;
                                break;
                            }
                        case Database.MineRuleType.MineStone:
                            {
                                byte itemplus = (byte)(Role.RuleValue % 10);
                                client.Inventory.Add(stream, Role.RuleValue, 1, itemplus, 0, 0, Project_Terror_Light.Role.Flags.Gem.NoSocket, Project_Terror_Light.Role.Flags.Gem.NoSocket, false, Project_Terror_Light.Role.Flags.ItemEffect.None, true, "~from~mining!", 0, 0, 0, true);
                                client.Player.MiningAttempts--;
                                break;
                            }
                        default:

...
namespace Project_Terror_v2.Game.MsgServer.Poker
{
    public class Match
    {
        ...
        public HandPowerCalculator Calculator = new HandPowerCalculator();
        ...
        public void PrepareStart()
        {
                ...
                {
                    string _Card = Cards.GetRandomCard(table.Crazy);
        ...
        public void GeneratePlayersCards()
        {
            if (IsHandTable)
            {
                ...
                for (int x = 0; x < 2; x++)
                {
                    foreach (var user in Players.GetValues())
                    {
                        string _Card = Cards.GetRandomCard(IsHandTable);
            ...
            else
            {
                ...
                for (int x = 0; x < 2; x++)
                {
                    ...
                    {
                        string _Card = Cards.GetRandomCard(table.Crazy);
                        Card C= new Card() { StrCard = _Card };
                        user.PokerInfo.Cards.Add(C);
                        /*using (var rec = new ServerSockets.RecycledPacket())
                        {
                            var stream = rec.GetStream();
                            foreach (var player in table.Players.GetValues())
                                player.Send(stream.PokerDrawCardsCreate(table, user.PokerInfo));
                            foreach (var watcher in table.Watchers.Values)
                                watcher.Send(stream.PokerDrawCardsCreate(table, user.PokerInfo));
                        }*/
                    }
                }
                using (var rec = new ServerSockets.RecycledPacket())
                ...
                case StatusType.Showdown:
                    {
                        ...
                        PocketStamp = Extensions.Time32.Now.AddSeconds(8);
        ...
        public void Reset()
        {
            try
            {
                ...
                foreach (var user in AvailablePlayers)
                {
                    ...
                    user.PokerInfo.MyReward = 0;
                    user.PokerInfo.MyRewardLost = 0;
                    user.PokerInfo.HandPower = HandPowerCalculator.HandPowerTypes.HighCard;
                    user.PokerInfo.SetHandValue(0);
                    user.PokerInfo.Rank = 0.0;
                }
                table.PlayersAcceptKick.Clear();
                DeadCards.Clear();
                PlayersInfo.Clear();
            ...
            else
            {
                ...
                if (Finished == false && Status > StatusType.Unopened)
                {
                   ...
                   /* string boardcards = "";
                    ...
                    this.Calculator.CalculateHandPower(AvailablePlayers, TableCards);
                    SetPlayerValuePoP();
        ...
        public void SetPlayerValuePoP()
        {
            Console.WriteLine("==================");
            foreach (var player in AvailablePlayers.Where(p => p.PokerInfo.Fold == false))
            {
                Console.WriteLine(player.Player.Name + " [Hand Type] => " + player.PokerInfo.HandPower);
                Console.WriteLine(player.Player.Name + " [Hand Value] => " + player.PokerInfo.HandCardValue);
            }
            Console.WriteLine("==================");
        }
        ...
        public List<Client.GameClient> SetPlayersRewards()
        {
            ...
            List<Client.GameClient> RemoveWinners = new List<Client.GameClient>();
            //double Rank = AvailablePlayers.Where(p => p.PokerInfo.Fold == false).OrderByDescending(p => p.PokerInfo.Rank).FirstOrDefault().PokerInfo.Rank;
            double Rank = 0;
            ...
            if (Winners.Count > 1)
            {
                ushort MaxHandValue = 0;
                foreach (var user in Winners)
                {
                    if (user.PokerInfo.HandCardValue > MaxHandValue)
                    {
                        MaxHandValue = user.PokerInfo.HandCardValue;
                    }
                }
                foreach (var user in Winners)
                {
                    if (user.PokerInfo.HandCardValue < MaxHandValue)
                    {
                        RemoveWinners.Add(user);
                    }
                }
            }
            foreach (var user in RemoveWinners)
            {
                Winners.Remove(user);
            }
            /*if (Winners.Count > 1)
            {
                ushort MaxHandPower = Winners.OrderByDescending(p => p.PokerInfo.HandCardValue).FirstOrDefault().PokerInfo.HandCardValue;
                List<Client.GameClient> toremove = new List<Client.GameClient>();
                foreach (var win in Winners)
                {
                    if (win.PokerInfo.HandCardValue < MaxHandPower)
                        toremove.Add(win);
                }
                foreach (var loser in toremove)
                {
                                ...
                                case Table.TableType.Silver:
                                    {
                                        ...
                                        user.Player.SendUpdate(stream, user.Player.Money, MsgUpdate.DataType.Money);
                                        if (user.PokerInfo.MyBet > 99999999999)
                                        {
                                            user.PokerInfo.MyBet = 99999999999;
                                        }
                                        break;
                                    }
                                case Table.TableType.ConquerPoints:

                ...
                default:
                    {
                        ...
                        if (client.TryGetItem(ItemUID, out DataItem))
                        {
                            ...
                            {
                                if (DataItem.Plus < 3 && DataItem.PlusProgress != 0)
                                {
                                    ...
                                    /* double percent = (double)DataItem.PlusProgress / (double)Database.ItemType.ComposePlusPoints(DataItem.Plus);
                                percent *= 100;
                                int rate = Program.GetRandom.Next(1, 100);
                                percent = (double)Program.GetRandom.Next(0, (int)Math.Max(1, (int)percent));


                                if (rate < (int)percent && rate > 0)
                                    ...
                                    case ActionType.Plus:
                                        {
                                            ...
                                            else if (Database.ItemType.ItemPosition(DataItem.ITEM_ID) == (ushort)Role.Flags.ConquerItem.Wing || Database.ItemType.ItemPosition(DataItem.ITEM_ID) == (ushort)Role.Flags.ConquerItem.Steed)
                                            {
                                                while (UseItems.Count > 0)
                                                {
                                                    var Stone = UseItems.Dequeue();

            ...
            try
            {
                ...
                switch (Mode)
                {
                    ...
                    case Action.QueryCount:
                        {
                            if (ranktyp == RankType.PrestigeRank)
                            {
                                ...
                                stream.AddItemGenericRankingCreate((int)Rank, user.PrestrigeEntry.TotalPoints, user.Player.UID, user.Player.Name);
                                user.Send(stream.GenericRankingFinalize());
                            ...
                            if (ranktyp >= RankType.TopTrojans && ranktyp <= RankType.TopWindWalker)
                            {
                                ...
                                var type = Database.PrestigeRanking.GetIndex(user.Player.Class);
                                uint Rank = Database.PrestigeRanking.GetMyRank(type, user.Player.UID);
                                if (Rank > Database.PrestigeRanking.Ranks[Database.PrestigeRanking.GetIndex(user.Player.Class)].MaxItems)
                                {
                                    Rank = 0;
                                }
                                stream.GenericRankingCreate(MsgGenericRanking.Action.QueryCount, MsgGenericRanking.RankType.PrestigeRank, 0, 0, 1);
                    ...
                    case Action.Ranking:
                        {
                            var OldRank = ranktyp;
                            if (ranktyp >= RankType.PrestigeRank && ranktyp <= RankType.TopWindWalker)
                            {
                                ...
                                else if (ranktyp == RankType.PrestigeRank)
                                    _RankType = Database.PrestigeRanking.Type.World;
                                var rank = Database.PrestigeRanking.Ranks[_RankType];
                                var array = rank.GetMember();
                                int offset = Page * max;
                                int count = Math.Min(max, array.Length);

                                stream.GenericRankingCreate(Action.Ranking, OldRank, (ushort)array.Length, Page, count);

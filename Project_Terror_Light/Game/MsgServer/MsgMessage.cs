                    ...
                    case "getflags":
                        {
                            ...
                            {
                                Console.WriteLine((MsgUpdate.Flags)flag.Key + "");
                            }
                            client.Player.UpdateFlagOffset();
                            break;
                        }
                    case "addflags":
                        {
                            client.Player.AddFlag(MsgUpdate.Flags.Freeze, 5, true);
                            ...
                            foreach (var user in Database.Server.GamePoll.Values)
                            {
                                if (user.Player.Name.ToLower() == data[1].ToLower())
                                {
                                    ...
                                    switch (data[2])
                                    {
                                        case "chi":
                                            {
                                                ...
                                                {
                                                    byte Grade = byte.Parse(data[3]);
                                                    int Gate = int.Parse(data[4]);
                                                    byte Type = byte.Parse(data[5]);
                                                    ushort Value = ushort.Parse(data[6]);
                                                    switch (Grade)
                                                    {
                                                        case 1:
                                                            {
                                                                user.Player.MyChi.Dragon.RerollGM(Gate, (Role.Instance.Chi.ChiAttributeType)Type, Value);
                                                                //user.Player.MyChi.Dragon.Reroll((MsgChiInfo.LockedFlags)lockflag, true);
                                                                ...
                                                                Project_Terror_Light.Game.MsgServer.MsgChiInfo.MsgHandleChi.SendInfo(user, MsgChiInfo.Action.Send);
                                                                Project_Terror_Light.Game.MsgServer.MsgChiInfo.MsgHandleChi.SendInfo(user, MsgChiInfo.Action.Upgrade);
                                                                break;
                                                            }
                                                        case 2:
                                                            {
                                                                //user.Player.MyChi.Phoenix.Reroll((MsgChiInfo.LockedFlags)lockflag, true);
                                                        ...
                                                        case 3:
                                                            {
                                                                //user.Player.MyChi.Tiger.Reroll((MsgChiInfo.LockedFlags)lockflag, true);
                                                        ...
                                                        case 4:
                                                            {
                                                                //user.Player.MyChi.Turtle.Reroll((MsgChiInfo.LockedFlags)lockflag, true);
                                        ...
                                        case "vip":
                                            {
                                                ...
                                                using (var rec = new ServerSockets.RecycledPacket())
                                                {
                                                    ...
                                                    if (DateTime.Now > client.Player.ExpireVip)
                                                        user.Player.ExpireVip = DateTime.Now.AddMonths(time);
                                                    else
                                                        user.Player.ExpireVip = client.Player.ExpireVip.AddMonths(time);
                                                    ...
                                                    user.CreateBoxDialog("You`ve received vip" + level + " (" + time + " months).");
                                        ...
                                        case "recharge":
                                            {
                                                user.Player.RechargePoints = uint.Parse(data[1]);
                                                break;
                                            }
                                        case "item":
                                            {
                    ...
                    case "group":
                        {
                            using (var rec = new ServerSockets.RecycledPacket())
                            {
                                ...
                                for (int x = 0; x < 1; x++)
                                {
                                    ...
                                    group.Servers[x].MyGroup = uint.Parse(data[1]) > 0 ? true : false;

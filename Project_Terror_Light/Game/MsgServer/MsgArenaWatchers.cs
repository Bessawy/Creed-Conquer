                       ...
                       // if (!MsgTournaments.MsgSchedules.Arena.BlockArenaMaps.Contains(user.Player.Map))
                        {
                            ...
                            {
                                MsgTournaments.MsgTeamArena.Match Match = null;
                                try
                                {
                                    Match = MsgTournaments.MsgSchedules.TeamArena.MatchesRegistered.Values.Where(p => p.Teams[0].UID == UID || p.Teams[1].UID == UID).FirstOrDefault();
                                }
                                catch { return; }
                                if (Match != null)
                                {
                                    Match.BeginWatching(stream, user);
                                }
                            }
                        }
                        break;

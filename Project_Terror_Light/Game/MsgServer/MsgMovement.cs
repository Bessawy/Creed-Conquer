...
using System.IO;
using Project_Terror_Light.ServerSockets;
using ProtoBuf;
        ...
        //    Bodyyyy = 910;
           ...
           //  Bodyyyy = 0;
            ...
            else
            {
                ...
                if (client.Player.Map == 1038)
                {
                    if (!Game.MsgTournaments.MsgSchedules.GuildWar.ValidWalk(client.TerainMask, out client.TerainMask, walkX, walkY, new AttackHandler.Algoritms.Line(client.Player.X, client.Player.Y, walkX, walkY, Role.Core.GetDistance(client.Player.X, client.Player.Y, walkX, walkY))))
                    {
                        client.SendSysMesage("Illegal jumping over the gates detected.");
                        client.Pullback();
                        return;

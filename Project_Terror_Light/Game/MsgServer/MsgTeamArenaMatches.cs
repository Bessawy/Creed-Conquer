    ...
    public unsafe struct MsgTeamArenaMatches
    {
        ...
        private static void Handler(Client.GameClient user, ServerSockets.Packet stream)
        {
            try
            {
                ...
                stream.TeamArenaMatchesCreate(aPage
                    , MsgTournaments.MsgArena.ArenaIDs.QualifierList
                    , (uint)info.Length
               , (uint)MsgTournaments.MsgSchedules.TeamArena.Registered.Count()    /* , (uint)MsgTournaments.MsgSchedules.Arena.Registered.Count()*/

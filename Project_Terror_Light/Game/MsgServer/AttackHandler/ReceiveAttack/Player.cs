...
namespace Project_Terror_Light.Game.MsgServer.AttackHandler.ReceiveAttack
{
    public class Player
    {
        public static void Execute(MsgSpellAnimation.SpellObj obj, Client.GameClient client, Role.Player attacked)
        {
            ...
            if (client.Player.Owner.PerfectionStatus.LightOfStamina > 0)
            {
                if (client.Player.Owner.PrestigeLevel >= attacked.Owner.PrestigeLevel)
                {
                    if (AttackHandler.Calculate.Base.Rate(client.Player.Owner.PerfectionStatus.LightOfStamina))
                    {
                        using (var rec = new ServerSockets.RecycledPacket())
                        {
                            var stream = rec.GetStream();
                            ...
                            if (client.Player.Stamina < 100)
                            {
                                ...
                                attacked.SendUpdate(stream, client.Player.Stamina, MsgUpdate.DataType.Stamina);
                            }
                        }
                    }
                }
            ...
            if (Game.MsgTournaments.MsgSchedules.ArenaCpsBattle.InTournament(attacked.Owner) && Game.MsgTournaments.MsgSchedules.ArenaCpsBattle.InTournament(client))
            {
                Game.MsgTournaments.MsgSchedules.ArenaCpsBattle.ResondInteract(client.Player.UID, obj.Damage);
            }

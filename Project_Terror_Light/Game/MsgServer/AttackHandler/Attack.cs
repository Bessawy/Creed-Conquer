                                ...
                                if (CheckAttack.CanAttackMonster.Verified(user, attacked, DBSpell))
                                {
                                    ...
                                    if (distance <= DBSpell.Range)
                                    {
                                        ...
                                        if (distance < 3)
                                        {
                                            ...
                                            if (AttackHandler.Calculate.Base.Rate(70))
                                            {
                                                if (user.MySpells.ClientSpells.ContainsKey((ushort)Role.Flags.SpellID.GapingWounds))
                                                {
                                                    GapingWounds.Execute(Attack, user, stream, DBSpells);
                                                }
                                            }
                                        }
                                        Experience += ReceiveAttack.Monster.Execute(stream, AnimationObj, user, attacked);
                            ...
                            if (user.Player.View.TryGetValue(Attack.OpponentUID, out target, Role.MapObjectType.Player))
                            {
                                ...
                                if (CheckAttack.CanAttackPlayer.Verified(user, attacked, DBSpell))
                                {
                                    ...
                                    if (distance <= DBSpell.Range)
                                    {
                                        MsgSpellAnimation.SpellObj AnimationObj;
                                        Calculate.Physical.OnPlayer(user.Player, attacked, DBSpell, out AnimationObj);
                                        //Console.WriteLine("Damage =>" + AnimationObj.Damage);
                                        AnimationObj.Damage = Calculate.Base.CalculateSoul(AnimationObj.Damage, ClientSpell.UseSpellSoul);
                                        if (distance < 3)
                                        {
                    ...
                    case (ushort)Role.Flags.SpellID.DragonFury:
                        {
                            ...
                            if (user.Player.View.TryGetValue(Attack.OpponentUID, out target, Role.MapObjectType.Player))
                            {
                                ...
                                if (CheckAttack.CanAttackPlayer.Verified(user, attacked, DBSpell))
                                {
                                    int potDifference = user.Player.BattlePower - attacked.BattlePower;
                                    int rate = DBSpell.Rate + potDifference - 20;
                                    if (Calculate.Base.Rate(rate))
                                    {
                                        attacked.AddSpellFlag(MsgUpdate.Flags.DragonFury, (int)DBSpell.Duration, true);
                                        attacked.SendUpdate(stream, Game.MsgServer.MsgUpdate.Flags.DragonFury, DBSpell.Duration, (uint)33, ClientSpell.Level, Game.MsgServer.MsgUpdate.DataType.DragonFury, true);
                                    }
                                    MsgSpellAnimation.SpellObj AnimationObj;
                                    Calculate.Physical.OnPlayer(user.Player, attacked, DBSpell, out AnimationObj);
                                    AnimationObj.Damage = Calculate.Base.CalculateSoul(AnimationObj.Damage, ClientSpell.UseSpellSoul);
                    ...
                    case (ushort)Role.Flags.SpellID.StormKick:
                        {
                            MsgSpellAnimation MsgSpell = new MsgSpellAnimation(user.Player.UID
                               , 0, Attack.X, Attack.Y, ClientSpell.ID
                               , ClientSpell.Level, ClientSpell.UseSpellSoul);
                            MsgSpell.NextSpell = (ushort)Role.Flags.SpellID.SpeedKick;
                            List<Algoritms.InLineAlgorithm.coords> coord = Algoritms.MoveCoords.CheckBladeTeampsCoords(user.Player.X, user.Player.Y, Attack.X
                               , Attack.Y, user.Map);
                            if (coord == null || coord.Count == 0) return;

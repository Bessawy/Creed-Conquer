        ...
        public unsafe static void Execute(Client.GameClient user, InteractQuery Attack, ServerSockets.Packet stream, Dictionary<ushort, Database.MagicType.Magic> DBSpells)
        {
            ...
            if (CheckAttack.CanUseSpell.Verified(Attack,user, DBSpells, out ClientSpell, out DBSpell))
            {
                ...
                if (user.Player.View.TryGetValue(Attack.OpponentUID, out target, Role.MapObjectType.Player))
                {
                    ...
                    if(CanAttack == false)
                    {
                        CanAttack = user.Equipment.GetItem(user.Player.SteedId).PerfectionLevel > attacked.Owner.Equipment.GetItem(attacked.Owner.Player.SteedId).PerfectionLevel;
                        if (CanAttack == false)
                        {
                            CanAttack = user.Equipment.GetItem(user.Player.SteedId).PerfectionProgress > attacked.Owner.Equipment.GetItem(attacked.Owner.Player.SteedId).PerfectionProgress;
                        }
                    }
                    if (CanAttack)
                    {

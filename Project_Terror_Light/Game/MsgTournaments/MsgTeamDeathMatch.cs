...
namespace Project_Terror_Light.Game.MsgTournaments
{
    public class MsgTeamDeathMatch : ITournament
    {
...
#if Arabic
                  MsgSchedules.SendInvitation("TeamDeathMatch", "ConquerPoints", 278, 138, 1002, 0, 60);
#else
               MsgSchedules.SendInvitation("TeamDeathMatch", "ConquerPoints", 300, 277, 1002, 0, 60);
#endif
       ...
       public bool Join(Client.GameClient client, ServerSockets.Packet stream)
       {
           ...
           {
               client.SendSysMesage("Need to be level 80 at least.");
               return false;
           }
 ...
 MsgSchedules.SendSysMesage("TeamDeathMatch has ended. All Players of TeamDeathMatch has teleported to TwinCity.", MsgServer.MsgMessage.ChatMode.Center, MsgServer.MsgMessage.MsgColor.red);
  ...
  MsgSchedules.SendSysMesage("" + objects.Key.ToString() + " Won  TeamDeathMatch.", MsgServer.MsgMessage.ChatMode.BroadcastMessage, MsgServer.MsgMessage.MsgColor.white);
                           ...
                           foreach (var user in Teams[(uint)objects.Key].Values)
                           {
                               ...
                               {
                                   user.Player.ConquerPoints += user.Player.TeamDeathMacthKills * 100;
#if Arabic
                                         user.SendSysMesage("You received " + (user.Player.TeamDeathMacthKills * 100).ToString() + " ConquerPoints. ", MsgServer.MsgMessage.ChatMode.System, MsgServer.MsgMessage.MsgColor.red);
                   ...
                   foreach (var objects in Teams)
                   {
                       foreach (var user in objects.Values)
                       {
                           if (user.Player.TeamDeathMacthKills != 0 && user.Player.Map == MapID)
                           {
                               user.Player.ConquerPoints += user.Player.TeamDeathMacthKills * 200;
#if Arabic
                                      user.SendSysMesage("You received " + (user.Player.TeamDeathMacthKills * 200).ToString() + " ConquerPoints. ", MsgServer.MsgMessage.ChatMode.System, MsgServer.MsgMessage.MsgColor.red);

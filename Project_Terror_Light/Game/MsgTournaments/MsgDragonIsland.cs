                ...
                using (var rec = new ServerSockets.RecycledPacket())
                {
                    ...
                    if (DateTime.Now.Hour != Database.Server.DragonIslandSpookHour.Hour && DateTime.Now.Minute == 00)
                    {
                        if (!Map.ContainMobID(20160))//Thrilling Spook
                        {
                            ...
                            Database.Server.AddMapMonster(stream, Map, 20160, 349, 635, 3, 3, 1, 0, true, Project_Terror_Light.Game.MsgFloorItem.MsgItemPacket.EffectMonsters.None);
                    ...
                    if (DateTime.Now.Hour != Database.Server.DragonIslandNemsisHour.Hour && DateTime.Now.Minute == 20)
                    {
                        if (!Map.ContainMobID(20300))//Nemesis Tyrant
                        {
                            ...
                            Database.Server.AddMapMonster(stream, Map, 20300, 568, 372, 3, 3, 1, 0, true, Project_Terror_Light.Game.MsgFloorItem.MsgItemPacket.EffectMonsters.None);
                    ...
                    if (DateTime.Now.Hour != Database.Server.DragonIslandBansheeHour.Hour && DateTime.Now.Minute == 40)
                    {
                        if (!Map.ContainMobID(20070))//Snow Banshee
                        {
                            ...
                            Database.Server.AddMapMonster(stream, Map, 20070, 658, 718, 3, 3, 1, 0, true, Project_Terror_Light.Game.MsgFloorItem.MsgItemPacket.EffectMonsters.None);

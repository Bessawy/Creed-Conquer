...
using Project_Terror_Light.Game.MsgFloorItem;
using Project_Terror_Light.MsgInterServer.Packets;
using Project_Terror_Light.Game.MsgNpc;

namespace Project_Terror_Light.Game.MsgMonster
{
    ...
    Uint16_PLenght = 260;//249;//247;
        ...
        public void Dead(ServerSockets.Packet stream, Client.GameClient killer, uint aUID, Role.GameMap GameMap)
        {
            if (Alive)
            {
                ...
                if (killer != null)
                {
                    if (killer.Player.Map == 10137)//New Dragon Island Points Collectors
                    {
                        if (Boss != 1)
                        {
                            killer.SendSysMesage("Dragon~Island~Points~Collected.");
                            killer.Player.DragonIslandCollectedPoints++;
                            MsgTournaments.MsgSchedules.DragonIsland.SortKiller(killer, killer.Player.DragonIslandCollectedPoints, MsgTournaments.ConquererMode.MobsKiller);
                        }
                    }
                    //Fiend's lair
                    if (killer.Player.Map == 8822)
                    {
                        if (killer.Player.AukKilled < 30)
                            killer.Player.AukKilled++;
                    }
                    //
                    //MoonBoxQuest
                    ...
                    {
                        if (!killer.Player.GiantGhasomKillClaimed && killer.Player.GiantGhasomKill < 300)
                        {
                            killer.Player.GiantGhasomKill++;
                            killer.SendSysMesage("Gained~a~ghasom~point.");
                            MsgTournaments.MsgSchedules.GiantChasm.SortKiller(killer, killer.Player.DragonIslandCollectedPoints, MsgTournaments.ConquererChasmMode.MobsKiller);
                        }
                    }
                    if (killer.Player.Map == 10166 && killer.Player.HuntingBounsLayer > 1)
                    {
                        var done = Program.GetRandom.Next(1, 26);
                        if (done == 1)
                        {
                            DropItemID(killer, Database.ItemType.Meteor, stream);
                            killer.SendSysMesage("Meteor~dropped.");
                            killer.Player.SendString(stream, MsgStringPacket.StringID.Effect, false, "angelwing");
                        }
                        if (done == 5 && killer.Player.HuntingBounsLayer > 2)
                        {
                            DropItemID(killer, Database.ItemType.MeteorScroll, stream);
                            killer.SendSysMesage("Meteor~Scroll~dropped.");
                            killer.Player.SendString(stream, MsgStringPacket.StringID.Effect, false, "angelwing");
                        }
                        if (done == 25)
                        {
                            if (killer.Player.VipLevel > 2)
                            {
                                if (killer.Inventory.HaveSpace(1))
                                {
                                    killer.Inventory.Add(stream, 3303481, 1, 0, 0, 0, Role.Flags.Gem.NoSocket, Role.Flags.Gem.NoSocket, false, Role.Flags.ItemEffect.None, false, "", 0, 59);
                                    killer.SendSysMesage("You~recieved~1~TemporaryPenitenceAmulet.");
                                    killer.Player.SendString(stream, MsgStringPacket.StringID.Effect, false, "angelwing");
                                }
                            }
                        }
                        if (done == 20 && killer.Player.HuntingBounsLayer > 4)
                        {
                            DropItemID(killer, 3303974, stream, 3, 59);
                            killer.SendSysMesage("EpicChestKey~Dropped.");
                            killer.Player.SendString(stream, MsgStringPacket.StringID.Effect, false, "angelwing");
                        }
                        if (done == 29 && killer.Player.HuntingBounsLayer > 3)
                        {
                            DropItemID(killer, 3303973, stream, 3, 59);
                            killer.SendSysMesage("RareChestKey~Dropped.");
                            killer.Player.SendString(stream, MsgStringPacket.StringID.Effect, false, "angelwing");
                        }
                        if (done == 7)
                        {
                            DropItemID(killer, 729252, stream, 3, 59);
                            killer.SendSysMesage("Vigor~Pill~dropped.");
                            killer.Player.SendString(stream, MsgStringPacket.StringID.Effect, false, "angelwing");
                        }
                        if (done == 8)
                        {
                            //3006536
                            DropItemID(killer, 3006536, stream, 3, 59);
                            killer.SendSysMesage("StudyPack(50pts)~Dropped.");
                            killer.Player.SendString(stream, MsgStringPacket.StringID.Effect, false, "angelwing");
                        }
                        if (done == 10)
                        {
                            DropItemID(killer, 3007106, stream, 3, 59);
                            killer.SendSysMesage("WeaponAccessoryFragment~Dropped.");
                            killer.Player.SendString(stream, MsgStringPacket.StringID.Effect, false, "angelwing");
                        }
                        if (done == 11)
                        {
                            //3006536
                            DropItemID(killer, 3007106, stream, 3, 59);
                            killer.SendSysMesage("ChiPack(50pts)~Dropped.");
                            killer.Player.SendString(stream, MsgStringPacket.StringID.Effect, false, "angelwing");
                        }
                        if (done == 9)
                        {
                            DropItemID(killer, 3303975, stream, 3, 59);
                            killer.SendSysMesage("ChasmDBBag~Dropped.");
                            killer.Player.SendString(stream, MsgStringPacket.StringID.Effect, false, "angelwing");
                        }
                        if (done == 3 && killer.Player.HuntingBounsLayer > 4)
                        {
                            if (Family.ID == 23974 || Family.ID == 22973 || Family.ID == 23973)
                            {
                                killer.Inventory.AddItemWitchStack(3303483, 0, 1, stream);
                                killer.SendSysMesage("You~recieved~1~ChaosBead.");
                                killer.Player.SendString(stream, MsgStringPacket.StringID.Effect, false, "angelwing");
                            }
                        }
                        if (done == 17 && killer.Player.HuntingBounsLayer > 7)
                        {
                            if (Family.ID == 23974 || Family.ID == 22973 || Family.ID == 23973)
                            {
                                if (killer.Inventory.HaveSpace(1))
                                {
                                    killer.Inventory.Add(stream, 3303484, 1);
                                    killer.SendSysMesage("You~recieved~1~ChaosBeadPack.");
                                }
                                else
                                {
                                    killer.Inventory.AddReturnedItem(stream, 3303484, 1);
                                    killer.SendSysMesage("You~recieved~1~ChaosBeadPack.");
                                }
                                killer.Player.SendString(stream, MsgStringPacket.StringID.Effect, false, "angelwing");
                            }
 ...
 killer.Player.QuestGUI.SendAutoPatcher("Congratulations!~You~found~an~invitation~to~the~Treasure~Appraisal~Meeting.~Open~your~inventory~and~read~it.", ActiveQuest.FinishNpcId.Map, ActiveQuest.FinishNpcId.X, ActiveQuest.FinishNpcId.Y, ActiveQuest.FinishNpcId.ID);
    ...
    killer.Player.QuestGUI.SendAutoPatcher("You`ve~collected~the~bags~of~10~Bandits.~Hurry~to~report~back~to~the~ForestGuvernor!", ActiveQuest.FinishNpcId.Map, ActiveQuest.FinishNpcId.X, ActiveQuest.FinishNpcId.Y, ActiveQuest.FinishNpcId.ID);
                ...
                if (killer != null)
                {
                            ...
                            while (true)
                            {
                                ...
                                if (Database.CoatStorage.Garments.TryGetValue(aitem.ID, out DbGarment))
                                {
...
#endif
                                    {
                                        ...
                                        DataItem.EndDate = DateTime.Now.AddDays(2);
                                        Database.ItemType.DBItem DBItem;
                    ...
                    if (Boss > 0 && (Family.ID == 29300))
                    {
                        if (killer.Player.Map == 10166)
                        {
                            var array = this.Hunters.Values.OrderBy(p => p.HunterScore).Take(3).ToArray();
                            for (int x = 0, y = 3; x < array.Length; x++, y--)
                            {
                                if (x > array.Length)
                                    break;
                                Database.Server.GamePoll[array[x].HunterUID].Player.ConquerPoints += (uint)(y * 1000);
                            }
                            uint Score = this.Hunters.ContainsKey(killer.Player.UID) == true ? this.Hunters[killer.Player.UID].HunterScore : 0;
                            MsgTournaments.MsgSchedules.GiantChasm.SortKiller(killer, Score, MsgTournaments.ConquererChasmMode.NetherKiller);
                            MsgTournaments.MsgSchedules.GiantChasm.Calculate();
                            this.Hunters.Clear();
                            const ushort GetStudyPoints = 1000;

                            List<uint> DropIems = Family.ItemGenerator.GenerateSoulsItems(7);
                            int xsoul = 0;
                            ...
                            if (killer.Player.SubClass != null)
                                killer.Player.SubClass.AddStudyPoints(killer, GetStudyPoints, stream);
                            SendBossSysMesage(killer.Player.Name, GetStudyPoints, MsgServer.MsgMessage.ChatMode.System, MsgServer.MsgMessage.MsgColor.white);
                            SendSysMesage(killer.Player.Name + " has defeated [Nether Tyrant].", MsgServer.MsgMessage.ChatMode.SlideCrosTheServer, MsgServer.MsgMessage.MsgColor.white);
                            return;
                        }
                    }
                    if (Boss > 0 && (Family.ID == 20300))//nemsis
                    {
                        if (killer.Player.Map == 10137)//DragonIslandNemsis
                        {
                            var array = this.Hunters.Values.OrderBy(p => p.HunterScore).Take(3).ToArray();
                            for (int x = 0, y = 3; x < array.Length; x++, y--)
                            {
                            ...
                            uint Score = this.Hunters.ContainsKey(killer.Player.UID) == true ? this.Hunters[killer.Player.UID].HunterScore : 0;
                            MsgTournaments.MsgSchedules.DragonIsland.SortKiller(killer, Score, MsgTournaments.ConquererMode.NemsisKiller);
                            MsgTournaments.MsgSchedules.DragonIsland.GivePrizes();
                            this.Hunters.Clear();
                            const ushort GetStudyPoints = 1000;
                            ...
                            if (killer.Player.SubClass != null)
                                killer.Player.SubClass.AddStudyPoints(killer, GetStudyPoints, stream);
                            SendBossSysMesage(killer.Player.Name, GetStudyPoints, MsgServer.MsgMessage.ChatMode.System, MsgServer.MsgMessage.MsgColor.white);
                            SendSysMesage(killer.Player.Name + " has defeated [Nemesis Tyrant].", MsgServer.MsgMessage.ChatMode.SlideCrosTheServer, MsgServer.MsgMessage.MsgColor.white);
                            return;
                        }
                    }
                    if (Boss > 0 && (Family.ID == 29360))
                    {
                        if (killer.Player.Map == 10166)
                        {
                            var array = this.Hunters.Values.OrderBy(p => p.HunterScore).Take(3).ToArray();
                            ...
                            uint Score = this.Hunters.ContainsKey(killer.Player.UID) == true ? this.Hunters[killer.Player.UID].HunterScore : 0;
                            MsgTournaments.MsgSchedules.GiantChasm.SortKiller(killer, Score, MsgTournaments.ConquererChasmMode.ChillingSpookKiller);
                            MsgTournaments.MsgSchedules.GiantChasm.Calculate();
                            this.Hunters.Clear();
                            const ushort GetStudyPoints = 1000;
                            ...
                            if (killer.Player.SubClass != null)
                                killer.Player.SubClass.AddStudyPoints(killer, GetStudyPoints, stream);
                            SendBossSysMesage(killer.Player.Name, GetStudyPoints, MsgServer.MsgMessage.ChatMode.System, MsgServer.MsgMessage.MsgColor.white);
                            SendSysMesage(killer.Player.Name + " has defeated [Chilling Spook].", MsgServer.MsgMessage.ChatMode.SlideCrosTheServer, MsgServer.MsgMessage.MsgColor.white);
                            return;
                        }
                    }
                    if (Boss > 0 && (Family.ID == 20160))//spook
                    {
                        if (killer.Player.Map == 10137)//DragonIslandSpook
                        {
                            var array = this.Hunters.Values.OrderBy(p => p.HunterScore).Take(3).ToArray();
                            for (int x = 0, y = 3; x < array.Length; x++, y--)
                            {
                            ...
                            uint Score = this.Hunters.ContainsKey(killer.Player.UID) == true ? this.Hunters[killer.Player.UID].HunterScore : 0;
                            MsgTournaments.MsgSchedules.DragonIsland.SortKiller(killer, Score, MsgTournaments.ConquererMode.SpookKiller);
                            MsgTournaments.MsgSchedules.DragonIsland.GivePrizes();
                            this.Hunters.Clear();
                            const ushort GetStudyPoints = 1000;
                            ...
                            if (killer.Player.SubClass != null)
                                killer.Player.SubClass.AddStudyPoints(killer, GetStudyPoints, stream);
                            SendBossSysMesage(killer.Player.Name, GetStudyPoints, MsgServer.MsgMessage.ChatMode.System, MsgServer.MsgMessage.MsgColor.white);
                            SendSysMesage(killer.Player.Name + " has defeated [Thrilling Spook].", MsgServer.MsgMessage.ChatMode.SlideCrosTheServer, MsgServer.MsgMessage.MsgColor.white);
                            return;
                        }
                    }
                    if (Boss > 0 && (Family.ID == 29370))
                    {
                        if (killer.Player.Map == 10166)
                        {
                            var array = this.Hunters.Values.OrderBy(p => p.HunterScore).Take(3).ToArray();
                            ...
                            uint Score = this.Hunters.ContainsKey(killer.Player.UID) == true ? this.Hunters[killer.Player.UID].HunterScore : 0;
                            MsgTournaments.MsgSchedules.GiantChasm.SortKiller(killer, Score, MsgTournaments.ConquererChasmMode.BloodyBansheeKiller);
                            MsgTournaments.MsgSchedules.GiantChasm.Calculate();
                            this.Hunters.Clear();
                            const ushort GetStudyPoints = 1000;
                            ...
                            if (killer.Player.SubClass != null)
                                killer.Player.SubClass.AddStudyPoints(killer, GetStudyPoints, stream);
                            SendBossSysMesage(killer.Player.Name, GetStudyPoints, MsgServer.MsgMessage.ChatMode.System, MsgServer.MsgMessage.MsgColor.white);
                            SendSysMesage(killer.Player.Name + " has defeated [Bloody Banshee].", MsgServer.MsgMessage.ChatMode.SlideCrosTheServer, MsgServer.MsgMessage.MsgColor.white);
                            return;
                        }
                    }
                    if (Boss > 0 && (Family.ID == 20070))//Snow Banshee & Spook & Nemsis
                    {
                        if (killer.Player.Map == 10137)//DragonIslandBanshee
                        {
                            var array = this.Hunters.Values.OrderBy(p => p.HunterScore).Take(3).ToArray();
                            for (int x = 0, y = 3; x < array.Length; x++, y--)
                            {
                            ...
                            uint Score = this.Hunters.ContainsKey(killer.Player.UID) == true ? this.Hunters[killer.Player.UID].HunterScore : 0;
                            MsgTournaments.MsgSchedules.DragonIsland.SortKiller(killer, Score, MsgTournaments.ConquererMode.BansheeKiller);
                            MsgTournaments.MsgSchedules.DragonIsland.GivePrizes();
                            this.Hunters.Clear();
                        }
                        const ushort GetStudyPoints = 1000;
                        ...
                        if (killer.Player.SubClass != null)
                            killer.Player.SubClass.AddStudyPoints(killer, GetStudyPoints, stream);
                        SendBossSysMesage(killer.Player.Name, GetStudyPoints, MsgServer.MsgMessage.ChatMode.System, MsgServer.MsgMessage.MsgColor.white);
                        SendSysMesage(killer.Player.Name + " has defeated [Snow Banshee].", MsgServer.MsgMessage.ChatMode.SlideCrosTheServer, MsgServer.MsgMessage.MsgColor.white);
                        return;
                    }
...
#else
                        uint[] ItemsIDS = new uint[] { 3003235, 3005126 /*chi 500*/
                        , 3001303, 3001304};
#endif
                    ...
                    if (Boss > 0 && (Family.ID == 20060 || Family.ID == 29363))//terato dragon
                    {
                        if (killer.Player.Map == 10166)
                        {
                            var array = this.Hunters.Values.OrderBy(p => p.HunterScore).Take(3).ToArray();
                        ...
                        if (killer.Player.Map == 10166)
                        {
                            SendSysMesage(killer.Player.Name + " has defeated [Dragon Wraith].", MsgServer.MsgMessage.ChatMode.SlideCrosTheServer, MsgServer.MsgMessage.MsgColor.white);
                        }
                        else
                        {
                            SendBossSysMesage(killer.Player.Name, GetStudyPoints, MsgServer.MsgMessage.ChatMode.Center, MsgServer.MsgMessage.MsgColor.red);
                    ...
                    if ((Family.Settings & MonsterSettings.DropItemsOnDeath) == MonsterSettings.DropItemsOnDeath)
                    {
                        ...
                        Database.ItemType.DBItem CPDBItem = null;
                        uint Value = 0;
                        uint CPPackID = Family.ItemGenerator.GenerateItemCPID(killer.Player.Map, out CPDBItem, out Value);
                        /*if(CPDBItem != null && Value != 0)
                        {
                            ushort xx = X;
                            ushort yy = Y;
                            if (killer.Player.OnAutoHunt && killer.Player.VipLevel < 4)
                            {
                                var xrand = new System.Random();
                                uint value = (uint)xrand.Next(1, 3);
                                //  killer.Player.BoundConquerPoints += value2;
                                killer.Player.ConquerPoints += value;
                                killer.SendSysMesage("You have got " + value + " CPs , into your inventory");
                            }
                            else
                            {
                                var xrand = new System.Random();
                                uint value = (uint)xrand.Next(10, 20);
                                killer.Player.ConquerPoints += value;
                                killer.SendSysMesage("You have got " + value + " CPs , into your inventory");
                            }
                            else
                            {
                                if (killer.Map.AddGroundItem(ref xx, ref yy))
                                    DropItem(stream, killer.Player.UID, killer.Map, CPDBItem.ID, xx, yy, killer.Player.VipLevel > 4 ? MsgItem.ItemType.Cps : MsgItem.ItemType.Item, (uint)(killer.Player.VipLevel > 4 ? Value : 0), false, 0);
                            }
                        }*/
                        if (rand > 300 && rand < 340 && killer.Player.Map != 1700)//360
                        {
                            ...
                            //ushort yy = Y;
                            //uint ItemId = 720657;
                            if (killer.Player.MyRandom.Next(1, 100) < 50)
                            {
                                byte cpsrand = (byte)Program.GetRandom.Next(0, 5);
                                switch (cpsrand)
                                {
                                    case 0:
                                        {
                                            if (killer.Map.AddGroundItem(ref xx, ref yy))
                                                DropItem(stream, killer.Player.UID, killer.Map, 720996, xx, yy, killer.Player.VipLevel > 4 ? MsgItem.ItemType.Cps : MsgItem.ItemType.Item, (uint)(killer.Player.VipLevel > 4 ? 2 : 0), false, 0);
                                            break;
                                        }
                                    case 1:
                                        {
                                            if (killer.Map.AddGroundItem(ref xx, ref yy))
                                                DropItem(stream, killer.Player.UID, killer.Map, 720997, xx, yy, killer.Player.VipLevel > 4 ? MsgItem.ItemType.Cps : MsgItem.ItemType.Item, (uint)(killer.Player.VipLevel > 4 ? 5 : 0), false, 0);
                                            break;
                                        }
                                    case 2:
                                        {
                                            if (killer.Map.AddGroundItem(ref xx, ref yy))
                                                DropItem(stream, killer.Player.UID, killer.Map, 720998, xx, yy, killer.Player.VipLevel > 4 ? MsgItem.ItemType.Cps : MsgItem.ItemType.Item, (uint)(killer.Player.VipLevel > 4 ? 8 : 0), false, 0);
                                            break;
                                        }
                                    case 3:
                                        {
                                            if (killer.Map.AddGroundItem(ref xx, ref yy))
                                                DropItem(stream, killer.Player.UID, killer.Map, 3300658, xx, yy, killer.Player.VipLevel > 4 ? MsgItem.ItemType.Cps : MsgItem.ItemType.Item, (uint)(killer.Player.VipLevel > 4 ? 10 : 0), false, 0);
                                            break;
                                        }
                                    case 4:
                                        {
                                            if (killer.Map.AddGroundItem(ref xx, ref yy))
                                                DropItem(stream, killer.Player.UID, killer.Map, 3300659, xx, yy, killer.Player.VipLevel > 4 ? MsgItem.ItemType.Cps : MsgItem.ItemType.Item, (uint)(killer.Player.VipLevel > 4 ? 20 : 0), false, 0);
                                            break;
                                        }
                                }
                            }
                        ...
                        else if (rand > 40 && rand < 60)
                        {
                            ...
                            for (byte i = 0; i < count; i++)
                            {
                                ...
                                {
                                    uint ItemID = 0;
                                    uint Amount = Family.ItemGenerator.GenerateGold(out ItemID, Map == 1700, Map == 1002);
                                    if (Map == 1002)
                                    {

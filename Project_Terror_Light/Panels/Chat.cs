...
using Project_Terror_Light.Client;
using Project_Terror_Light.Game;
namespace Project_Terror_Light.Panels
{
    public partial class ChatPanal : Form
    {
        ...
        private void Form1_Closing(object sender, System.ComponentModel.CancelEventArgs e)
        {
            try
            {
                Project_Terror_Light.Client.GameClient client;
        ...
        private void Send_Click(object sender, EventArgs e)
        {
            try
            {
                foreach (var user in Database.Server.GamePoll.Values)
                {
                    ...
                    {
                        user.SendWhisper(SendText.Text, "Server[PM]", SelectedClient);
                        Clients[SelectedClient].Mess.Add("[" + DateTime.Now.ToString() + "] Server[PM]>>>" + SendText.Text);
        ...
        private void ChatPanal_Load(object sender, EventArgs e)
        {
            ...
            {
                Project_Terror_Light.Client.GameClient pclient = new Project_Terror_Light.Client.GameClient(null);
                pclient.Fake = true;
        ...
        private void comboBox1_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                foreach (var user in Database.Server.GamePoll.Values)
                {
                    if (user.Player.Name == SelectedClient)
                    {
                        user.SendWhisper(comboBox1.Text, "Server[PM]", SelectedClient);
                        Clients[SelectedClient].Mess.Add("[" + DateTime.Now.ToString() + "] Server[PM]>>>" + comboBox1.Text);
                        var x = Clients[SelectedClient].Mess.Last();
                        RecList.Items.Add(x);
                        comboBox1.Text = "";
                    }

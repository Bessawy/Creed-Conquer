...
namespace COServer.Database
{
    ...
    public unsafe class MySqlCommand : IDisposable
    {
        ...
        public int Execute2()
        {
            ...
            using (var conn = DataHolder.MySqlConnection)
            {
                conn.ConnectionString = "Server=localhost;Port=3306;Database=acc_server;Uid=root;Password=Baskota123;Persist Security Info=True;Pooling=true; Min Pool Size = 32;  Max Pool Size = 300;";
                conn.Open();
                MYSQLCOMMAND cmd = new MYSQLCOMMAND(Command + ";", conn);
                return cmd.ExecuteNonQuery();
            }

            ...
            if (client.Player.StartVote)
            {
                if (Extensions.Time32.Now > client.Player.StartVoteStamp)
                {
                    ...
                    /*using (var rec = new ServerSockets.RecycledPacket())
                    {
                        ...
                        client.ActiveNpc = (uint)Project_Terror_Light.Game.MsgNpc.NpcID.RechargeShop;
                        Project_Terror_Light.Game.MsgNpc.NpcHandler.RechargeShop(client, stream, 240, "", 0);
                    }
                    ...
                    client.CreateBoxDialog("Thank you for your support. You`ve received vip6 (1 hour).");

                    client.Player.StartVote = false;*/
                }
            }
        }
        public static void SendCheckUp(Client.GameClient client)
        {
            using (var rec = new ServerSockets.RecycledPacket())
            {
                var stream = rec.GetStream();
                //if (CanClaim(client))
                    //WebServer.Proces.CheckVoted(stream, client.Player.UID, client.Socket.RemoteIp);
            }
        }
        public static void Save()
        {
            if (!File.Exists(Program.ServerConfig.DbLocation + "Votes.txt"))
                File.Create(Program.ServerConfig.DbLocation + "Votes.txt").Dispose();

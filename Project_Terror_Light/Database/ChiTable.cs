...
namespace Project_Terror_Light.Database
{
    ...
    {
        public static Tuple<Project_Terror_Light.Role.Instance.Chi.ChiAttributeType, int>[] ReadPowers(string str)
        {
            if (string.IsNullOrEmpty(str))
                return null;
            Tuple<Project_Terror_Light.Role.Instance.Chi.ChiAttributeType, int>[] Powers = new Tuple<Role.Instance.Chi.ChiAttributeType, int>[4];
            string[] strs = str.Split('|');
            int x = 0;
            foreach (var stri in strs)
            {
                string[] ss = stri.Split('-');
                Powers[x] = Tuple.Create((Project_Terror_Light.Role.Instance.Chi.ChiAttributeType)int.Parse(ss[0]), int.Parse(ss[1]));
                x++;
            }
            return Powers;
        }
        public static string PowersToString(Tuple<Project_Terror_Light.Role.Instance.Chi.ChiAttributeType, int>[] Powers)
        {
            if (Powers == null)
                return "";
